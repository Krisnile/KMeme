						...
						<view 
							...
							<view class="image-container">
								...
								<view class="image-overlay">
									<up-icon name="heart-fill" size="20" :color="iconColors.heartFill"></up-icon>
								</view>
							</view>
							<view class="item-info">
								...
								<view class="item-actions">
									<view class="action-btn" @tap.stop="uncollectImage(item, index)">
										<up-icon name="heart-fill" size="16" :color="iconColors.heartFill"></up-icon>
										<text>取消收藏</text>
									</view>
									<view class="action-btn" @tap.stop="shareImage(item)">
										<up-icon name="share" size="16" :color="iconColors.share"></up-icon>
										<text>分享</text>
									</view>
								</view>
							</view>
...
const uncollectImage = (item, index) => {
	uni.showModal({
		...
		success: (res) => {
			if (res.confirm) {
				// 从收藏列表中移除
				const originalIndex = collectList.value.findIndex(collectItem => 
					collectItem.img === item.img
				)
				if (originalIndex > -1) {
