	...
	<view class="upload-container">
		...
		<view class="content">
				...
				<view class="image-grid">
					<view 
						...
						<view class="delete-btn" @tap.stop="deleteImage(index)">
							<up-icon name="close" size="16" :color="iconColors.close"></up-icon>
						</view>
					</view>
			...
			<view class="form-section" v-if="imageList.length > 0">
				<view class="section-title">
					<up-icon name="edit-pen" size="20" :color="iconColors.editPen"></up-icon>
					<text>图片信息</text>
				</view>
			...
			<view class="tips-section">
				<view class="tips-title">
					<up-icon name="info-circle" size="18" :color="iconColors.infoCircle"></up-icon>
					<text>使用说明</text>
				</view>
	...
	uni.chooseImage({
		...
		success: (res) => {
			...
			if (!formData.title && imageList.value.length === 1) {
				formData.title = `我的图片 ${new Date().getMonth() + 1}-${new Date().getDate()}`
			}
		},
		fail: () => {
			uni.showToast({
